...
class SLL {
  ...
  display() {
    // for this one, it's similar to the ones before, where we'll be needing another runner to go through a while loop that runs through our SLL
    let runner = this.head;
    // since we are returning a list, we need to make a new variable that currently holds an empty list
    let new_list = [];
    // we will be running our while loop until runner == null
    while (runner !== null) {
      // for every loop, we want to push the runner.data value to the new_list
      new_list.push(runner.data);
      // to continue through the loop, we need to ovewrite runner to equate runner.next
      runner = runner.next;
    }
    // afterwards, we need to return our new_list
    return new_list;
  }
}
